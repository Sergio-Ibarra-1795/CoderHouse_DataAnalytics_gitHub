-- a) Constructor standings for a year
CREATE FUNCTION dbo.GetConstructorStandings(@year INT)
RETURNS TABLE 
AS
RETURN 
(
    ...
    WHERE raceId IN (SELECT raceId FROM Races WHERE year_race = @year)
    ORDER BY position
);
...
SELECT * 
FROM dbo.GetConstructorStandings(2021);
...
CREATE FUNCTION dbo.GetDriverStandings(@year INT)  
RETURNS TABLE
AS
RETURN
(
...
CREATE FUNCTION dbo.GetDriverFinalStandings(@driverName VARCHAR(100))
RETURNS TABLE
AS 
RETURN
(
    SELECT year_race, position 
    FROM Races R
